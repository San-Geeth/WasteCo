...
<div class="product-section mt-150 mb-150">
    <div class="container">
                ...
                <div class="p-1 bg-light rounded rounded-pill shadow-sm mb-4">
                    <div class="input-group">
                        <input type="search" placeholder="What're you searching for?" aria-describedby="button-addon1"
                               class="form-control border-0 bg-light" name="bidSearch">
                        <div class="input-group-append">
                            <button id="button-addon1" type="submit" class="btn btn-link text-primary" name="searchBid">
                                <i
        ...
        <div class="row product-lists">
            <?php
            if (isset($_POST['searchBid'])) {
                $name = $_POST['bidSearch'];
                $con = mysqli_connect("localhost:3306", "root", "", "wasteco");
                $sql = "SELECT * FROM bidp WHERE title LIKE '$name%'";
                $query_run = mysqli_query($con, $sql);
                while ($row = mysqli_fetch_assoc($query_run)) {
                    if ($row['status'] == 'open') {
                        $image = "biddingImages/" . $row['image'];
                        $_SESSION['bidID'] = $row['bidID'];
                        bidComponent($row['title'], $row['price'], $row['qty'], $image, $row['type'], $row['location'], $row['bidID']);
                    }

                }
            } else {
                $con = mysqli_connect("localhost:3306", "root", "", "wasteco");
                $sql = "SELECT * FROM bidp ORDER BY bidID DESC";
                $query_run = mysqli_query($con, $sql);
                while ($row = mysqli_fetch_assoc($query_run)) {
                    if ($row['status'] == 'open') {
                        $image = "biddingImages/" . $row['image'];
